...
CUSTOM_SINK_ICONS=(" " " " "蓼 " )  # Custom sink icons in index of sink order
...
function showListDevice() {
    echo $(pactl list sinks | grep 'device.description' | awk -F = '{print $2}')
}
function showCurrentDevice() {
    echo $(pacmd list-sinks | grep -A 55 '* index:' | grep 'device.description' | awk -F = '{print $2}')
}
...
function output() {
    ...
    echo "${curVol}|${curSink}|${isMuted}|$(showCurrentDevice)"
}
